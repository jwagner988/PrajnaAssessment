...
function handleSearch(e){
  ...
  if (search !== ''){
  const results = restaurants.filter((curr) => {
    return curr.name.toLowerCase().startsWith(search.toLowerCase())
  })
      ...
      <form>
        ...
        <label>
          ...
          <select onChange={handleSelect}>
            ...
            {cuisines(data).map(rest => {
            return <option value={rest}>{rest}</option>
          })}
          </select>
        </label>
      </form>
